...
% Relevant outputs: Figs. 2b, 4bc, 5, 6, S6
...
addpath(fullfile(project_root, 'loadsvg_2'));
addpath(fullfile(project_root, 'annotated'));
addpath(fullfile(project_root, 'seismic'));
addpath(fullfile(project_root, 'DrosteEffect-BrewerMap-3.2.5.0'));
...
%% Manuscript Figure 2b: Average depth versus longitude 
% close all;
...
%% Manuscript Figure S7: Correlating propagation speed with A & T
...
%% Propagation speed binned by depth
% close all;
...
%% Manuscript Figure 4bc: T and A over time 
% close all;
...
www  = unique(all_detections.Type);    
cols = lines(numel(www));               
...
a_cmap_full = brewermap(length(a_grid)+3, 'Reds');
a_cmap = a_cmap_full(4:end, :);

h1_cmap_full = brewermap(length(h1_grid)+4, 'Blues');
h1_cmap = h1_cmap_full(5:end, :);

T_model = nan(length(a_grid), length(h1_grid));
A_model = nan(length(a_grid), length(h1_grid));
...
for aa = 1:length(a_grid)
    ...
    for hh = 1:length(h1_grid)
        ...
        % Calculating T and V
        chosen_pressure = T_smooth;

        % Original method for calculating period
        % T_grid(hh)  = T; 

        % New method (using period = time between crest and trough)
        crest_idx = find(s==0);
        crest = chosen_pressure(crest_idx);
        before_peak = chosen_pressure(1:crest_idx);
        [trough, trough_idx] = min(before_peak);
        s_trough = s(trough_idx);
        s_crest = s(crest_idx);
        L_new = s_crest-s_trough;

        T_model(aa,hh) = L_new/c1;
        A_model(aa,hh) = crest-trough;
        
    end

end

for hp = 1:length(h1_grid)
    plot(T_model(:,hp), A_model(:,hp), ...
        'DisplayName',sprintf('h_{1} = %g m', h1_grid(hp)), ...
        'Color', h1_cmap(hp,:), ...
        'LineWidth', 3); %h1_grid(hp)/17)
end

for ap = 1:length(a_grid)
    plot(T_model(ap,:), A_model(ap,:), ...
        'DisplayName',sprintf('a = %g m', a_grid(ap)), ...
        'Color', a_cmap(ap,:), ...
        'LineWidth',3)
end

% observed data from OBP
Tvals = seconds(all_detections.period);
Avals = all_detections.amps;
loglog(Tvals, Avals, 'k.', 'MarkerSize', 14, 'DisplayName','Observed data');
p_unbin = polyfit(log10(Tvals), log10(Avals), 1)
xfit = linspace(min(Tvals), max(Tvals), 100);
yfit = 10.^(polyval(p_unbin, log10(xfit)));
loglog(xfit, yfit, 'k:','LineWidth',2, 'DisplayName','Observed data trend')
...
xlim([250 1000])
ylim([40 1700])
%% Manuscript Figure 6: Fortnightly cycle
...
plot(tmd_output.t_otps, tmd_output.z_otps, 'Color',	[0.6350, 0.0780, 0.1840])
xline(edges1, 'k')
...
function [c0store,kstore,alpha,beta,I4,I5]=ComputeCoefficients(h1,h2,gp,g)
    ...
    z=[0:dz:H];
    ii=0;
    for ik=-4:0.005:-1, % this creates steps, and we can smooth out using 0.001
        ii=ii+1;
        k=10^ik;
        kstore(ii)=k;
