...
for aa = 1:length(a_grid)
    ...
    for hh = 1:length(h1_grid)
        ...
        T_grid(hh) = T; % currently L/c1 - change to peak to trough
        A_grid(hh) = max(Total);
    ...
    % hold on
    % loglog(xfit, yfit, '-','LineWidth',2, 'DisplayName',sprintf('a = %g m', a_grid(aa)))
    hold on
    grid on
end
...
%% Figure S8 - pH/pSH ratio dependence on h1 and a
...
%% Part 4 Showing how T varies with h1 (Eqn. 52 of SI)
